...
service / on new http:Listener(9090) {
    ...
    isolated resource function get fhir/r4/Patient(http:RequestContext ctx, http:Request request) returns @http:Payload {mediaType: ["application/fhir+json", "application/fhir+xml"]} json|xml|r4:FHIRError|error {
        ...
        if process is error {
            log:printError("Error in source connection processing", process);
        }
    ...
    resource function post fhir/r4/Patient(http:RequestContext ctx, http:Request request) returns @http:Payload {mediaType: ["application/fhir+json", "application/fhir+xml"]} json|r4:FHIRError {
